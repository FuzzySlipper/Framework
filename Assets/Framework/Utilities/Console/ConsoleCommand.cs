...
namespace PixelComrades.Debugging
{
    ...
    public class ConsoleCommand
    {
        ...
        private ConsoleCommand(MethodInfo method, CommandAttribute attribute, Type owner)
        {
            this.method = method;
            Initialize(attribute, owner);
        }

        private ConsoleCommand(PropertyInfo property, CommandAttribute attribute, Type owner)
        {
            this.property = property;
            Initialize(attribute, owner);
        }

        private ConsoleCommand(FieldInfo field, CommandAttribute attribute, Type owner)
        {
            this.field = field;
            Initialize(attribute, owner);
        }
        ...
        public static ConsoleCommand Create(MethodInfo method, Type type)
        {
            CommandAttribute attribute = method.GetCommand();

            if (attribute == null) return null;

            ConsoleCommand consoleCommand = new ConsoleCommand(method, attribute, type);
            return consoleCommand;
        }

        public static ConsoleCommand Create(PropertyInfo property, Type type)
        {
            CommandAttribute attribute = property.GetCommand();

            if (attribute == null) return null;

            ConsoleCommand consoleCommand = new ConsoleCommand(property, attribute, type);
            return consoleCommand;
        }

        public static ConsoleCommand Create(FieldInfo field, Type type)
        {
            CommandAttribute attribute = field.GetCommand();

            if (attribute == null) return null;

            ConsoleCommand consoleCommand = new ConsoleCommand(field, attribute, type);
            return consoleCommand;
        }

...
namespace PixelComrades {
    ...
    public class SpriteDissolveParticlesComponent : IComponent, IReceive<CollisionEvent>, IReceive<DeathEvent> {
        
        private CachedUnityComponentReference<SpriteRenderer> _spriteRenderer;
        private CollisionEvent _lastCollision;
        private float _currentHealth;

        public SpriteDissolveParticlesComponent(SpriteRenderer spriteRenderer) {
            _spriteRenderer = new CachedUnityComponentReference<SpriteRenderer>(spriteRenderer);
        }
        ...
        public void Handle(DeathEvent arg) {
            UnityFxManager.StartFx(this.GetEntity(), _lastCollision, _spriteRenderer, arg.OverKill);
        }

        public SpriteDissolveParticlesComponent(SerializationInfo info, StreamingContext context) {
            _currentHealth = info.GetValue(nameof(_currentHealth), _currentHealth);
            _spriteRenderer = info.GetValue(nameof(_spriteRenderer), _spriteRenderer);
        }

        public void GetObjectData(SerializationInfo info, StreamingContext context) {
            info.AddValue(nameof(_currentHealth), _currentHealth);
            info.AddValue(nameof(_spriteRenderer), _spriteRenderer);
        }
    }

    public class HitParticlesComponent : IComponent, IReceive<CollisionEvent>, IReceive<EnvironmentCollisionEvent>, IReceive<PerformedCollisionEvent> {
        ...
        public void Handle(CollisionEvent arg) {
            UnityFxManager.main.EmitHitParticles(arg.HitPoint, arg.HitNormal, _color);
        }

        public void Handle(EnvironmentCollisionEvent arg) {
            UnityFxManager.main.EmitHitParticles(arg.HitPoint, arg.HitNormal, _color);
            UnityFxManager.main.EmitDecalParticles(arg.HitPoint, arg.HitNormal, _color);
        }
        ...
        public HitParticlesComponent(SerializationInfo info, StreamingContext context) {
            _color = info.GetValue(nameof(_color), _color);
        }

        public void GetObjectData(SerializationInfo info, StreamingContext context) {
            info.AddValue(nameof(_color), _color);
        }
    }

    public class ParticleTrailComponent : IComponent, IReceive<ProjectileDespawned>, IReceive<ProjectileSpawned> {
        ...
        public void Handle(ProjectileSpawned arg) {
            World.Get<ProjectileTrailSystem>().TrailComponents.Add(this);
        }

        public void Handle(ProjectileDespawned arg) {
            World.Get<ProjectileTrailSystem>().TrailComponents.Remove(this);
            LastTime = -1;
        }
    ...
    public class ProjectileTrailSystem : SystemBase, IMainSystemUpdate {

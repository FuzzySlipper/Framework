...
namespace PixelComrades {
    public class ProtectCameraFromWallClip : MonoBehaviour {
        ...
        public float sphereCastRadius = 0.1f;           // the radius of the sphere used to test for object between camera and unit
        public bool visualiseInEditor;                  // toggle for visualising the algorithm through lines for the raycast in the editor
        public float closestDistance = 0.5f;            // the closest distance the camera can be from the unit
        public bool protecting { get; private set; }    // used for determining if there is an object between the unit and the camera
        ...
        private float m_CurrentDist;              // the current distance from the camera to the unit
        private Ray m_Ray = new Ray();                        // the ray used in the lateupdate for casting between the camera and the unit
        private RaycastHit[] m_Hits;              // the hits between the camera and the unit
        private RayHitComparer m_RayHitComparer;  // variable to compare raycast hit distances
        private CameraThirdPerson _camera;

        private void Start() {
            // find the camera in the object hierarchy
            m_Cam = GetComponentInChildren<Camera>().transform;
            m_Pivot = m_Cam.parent;
            _camera = GetComponent<CameraThirdPerson>();
        ...
        private void LateUpdate() {
            // initially set the unit distance
            float targetDist = _camera.CamDistance;
            ...
            if (initialIntersect) {
                ...
                m_Hits = Physics.RaycastAll(m_Ray, _camera.CamDistance - sphereCastRadius);
            }
            else {
                // if there was no collision do a sphere cast to see if there were any other collisions
                m_Hits = Physics.SphereCastAll(m_Ray, sphereCastRadius, _camera.CamDistance + sphereCastRadius);
            }
            ...
            m_CurrentDist = Mathf.Clamp(m_CurrentDist, closestDistance, _camera.CamDistance);
